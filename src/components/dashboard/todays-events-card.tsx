...
function EventRow({ event, onClick }: { event: Event; onClick?: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-start gap-3 rounded-xl px-1 py-1 text-left transition hover:bg-white/5"
    >
      <div
      ...
      <div className="min-w-0 pt-0.5">
        ...
        <p className="mt-0.5 text-sm text-zinc-400">{eventDetail(event)}</p>
      </div>
    </button>
  );
}
